...
public interface UserFootRepository extends JpaRepository<UserFoot, Long> {
    ...
    Page<Long> findReadArticleIdsByUserId(@Param("userId") Long userId,
                                          Pageable pageable);

    @Query("SELECT uf.documentId FROM UserFoot uf WHERE uf.userId = :userId AND uf.documentType = 1 AND uf.readStat = 1")
    Page<Long> findCollectedArticleIdsByUserId(@Param("userId") Long userId,
                                               Pageable pageable);

    @Query(value = "SELECT new com.shanzha.service.dto.SimpleUserInfoDTO(u.id, u.login, u.imageUrl) " +
    ...
    Page<SimpleUserInfoDTO> findPraisedUsers(@Param("articleId") Long articleId,
